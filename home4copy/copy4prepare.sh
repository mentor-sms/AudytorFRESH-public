...
clean_home() {
  ...
  if [ "$dry" -ne 1 ]; then
    find "$target" -type f -name "*lab*" -print0 | while IFS= read -r -d '' file; do
      echo_info "Removing file: $file"
      rm -f "$file" || echo_info "Warning: Could not remove $file"
    done
  else
    find "$target" -type f -name "*lab*" -print | while IFS= read -r file; do
      echo_info "Dry run: Would remove $file"
    done
  fi
  if [ -f "$target/.mentor/prepare.lab.step" ]; then
    echo_info "Removing prepare.lab.step file"
    if [ "$dry" -ne 1 ]; then
      rm -f "$target/.mentor/prepare.lab.step" || echo_error $LINENO "Failed to remove prepare.lab.step file"
    else
      echo_info "Dry run: Would remove $target/.mentor/prepare.lab.step"
    fi
  else
...
handle_bclear() {
  ...
  done < <(find "$target_root" -name "*.lab.bak" -type f 2>/dev/null)
...
run_rsync() {
  ...
  dry_run_exit_code=0
  local dry_exit_tmp
  dry_exit_tmp="$(mktemp)" || echo_error $LINENO "Nie mozna utworzyc pliku tymczasowego"
  local line
  while IFS= read -r line; do
    ...
    $dry_rsync_cmd 2>&1
    echo "RSYNC_EXIT_CODE:$?" >&2
  ) 2> >(
    while IFS= read -r error_line; do
      if [[ "$error_line" == RSYNC_EXIT_CODE:* ]]; then
        printf "%s" "${error_line#RSYNC_EXIT_CODE:}" >"$dry_exit_tmp"
      else
        echo "$error_line" >&2
      fi
    done
  )
  if [ -s "$dry_exit_tmp" ]; then
    dry_run_exit_code="$(cat "$dry_exit_tmp" 2>/dev/null || echo 0)"
  else
    dry_run_exit_code=0
  fi
  rm -f "$dry_exit_tmp" || true
  ...
  if [ "$dry" -ne 1 ]; then
    ...
    rsync_exit_code=0
    local rsync_exit_tmp
    rsync_exit_tmp="$(mktemp)" || echo_error $LINENO "Nie mozna utworzyc pliku tymczasowego"
    while IFS= read -r line; do
      echo "$line" # Show live output
      local first_part second_part
      first_part="${line%% *}"
      ...
      while IFS= read -r error_line; do
        if [[ "$error_line" == RSYNC_EXIT_CODE:* ]]; then
          printf "%s" "${error_line#RSYNC_EXIT_CODE:}" >"$rsync_exit_tmp"
        else
          echo "$error_line" >&2
        fi
      done
    )
    if [ -s "$rsync_exit_tmp" ]; then
      rsync_exit_code="$(cat "$rsync_exit_tmp" 2>/dev/null || echo 0)"
    else
      rsync_exit_code=0
    fi
    rm -f "$rsync_exit_tmp" || true
...
mnt_mnt() {
  local local_mnt
  local_mnt="$1"
  echo_info "Przygotowywanie punktu montowania $local_mnt"
  if [ ! -d "$local_mnt" ]; then
    echo_info "Tworzenie katalogu montowania $local_mnt"
    mkdir -p "$local_mnt" || echo_error $LINENO "Nie udało się utworzyć katalogu montowania $local_mnt"
  fi
  is_mounted "$from" "$local_mnt"
  if [ $last_is_mounted -eq 1 ]; then
    echo_info "$from jest już zamontowany w $local_mnt"
    local actual_mntdir
    ...
    if [ -n "$existing_mount" ]; then
      ...
      echo_info "Montuję urządzenie $from w $local_mnt"
      sudo mount "$from" "$local_mnt" || echo_error $LINENO "Nie udało się zamontować $from w $local_mnt"
      do_umount=1
      set_from "$local_mnt"
    fi
  fi
